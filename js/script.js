    ...
    // *--- FUNCTIONS ---*
        ...
        function generateTable() {
            ...
            for(let i = 0; i < displayRecords.length; i++){  // loops through data and appends to table
                ...
                tr.append(`<td><img src="${displayRecords[i].thumb}"/></td>`)
        ...
        function apply_pagination() {
            $pagination.twbsPagination({ // Calls plugin
                ...
                onPageClick: function(event, page) {  // Handles paging 
                    displayRecordsIndex = Math.max(page - 1, 0) * recPerPage;  // Gets index of records from data
                    endRec = (displayRecordsIndex) + recPerPage; // Sets record cutoff for page
                    displayRecords = artistRlsData.results.slice(displayRecordsIndex, endRec);  // Assigns paginated results to variable
                    // console.log(artistRlsDataIndex);
                    generateTable();  // displayRecords now defined and can be used to populate table.
                }
            })
        };
